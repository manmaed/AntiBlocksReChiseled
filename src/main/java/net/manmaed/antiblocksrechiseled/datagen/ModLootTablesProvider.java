...
public class ModLootTablesProvider extends LootTableProvider {
    ...
    public static class ModBlockLoot extends BlockLootSubProvider {
        ...
        protected Iterable<Block> getKnownBlocks() {
            ...
            ABRCBrightColors.getBlocks().getEntries().stream().map(RegistryObject::get).forEach((b) -> blocks.add(b.defaultBlockState().getBlock()));
            ABRCWoolColors.getBlocks().getEntries().stream().map(RegistryObject::get).forEach((b) -> blocks.add(b.defaultBlockState().getBlock()));
            ABRCSlabs.getBlocks().getEntries().stream().map(RegistryObject::get).forEach((b) -> blocks.add(b.defaultBlockState().getBlock()));
            ABRCStairs.getBlocks().getEntries().stream().map(RegistryObject::get).forEach((b) -> blocks.add(b.defaultBlockState().getBlock()));
            ABRCButtons.getBlocks().getEntries().stream().map(RegistryObject::get).forEach((b) -> blocks.add(b.defaultBlockState().getBlock()));
            ABRCPressurePlates.getBlocks().getEntries().stream().map(RegistryObject::get).forEach((b) -> blocks.add(b.defaultBlockState().getBlock()));
            return blocks;
        }
    }
}
